...
def train(
    ...
    AUC_ROC, BEST_Fc1, BEST_AUC_F1_K = evaluate(model, config, test_loader1, test_loader2, nsample=20, scaler=1, mean_scaler=0, foldername="",epoch_number = "",name="")
...
def evaluate(model, config, test_loader1, test_loader2, nsample=20, scaler=1, mean_scaler=0, foldername="",epoch_number = "",name=""):
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader1, mininterval=5.0, maxinterval=50.0) as it:
            ...
            with open(
                ...
                all_mse = torch.cat(all_mse, dim=0).to("cpu")
        ...
        gt = np.load("/content/drive/MyDrive/data/dataset/" + config["dataset"] + '/' + config["dataset"] + '_test_label.npy')
        test_scores= all_mse.numpy()
        ...
        eval_tool = my_eval(gt=gt, test_scores=test_scores)
        AUC_ROC, BEST_Fc1, BEST_AUC_F1_K = eval_tool.eval_result()
    return AUC_ROC, BEST_Fc1, BEST_AUC_F1_K
